...
export const TARGET_PERIODS=Object.freeze(['7D','30D','60D','90D','365D','ALL']);
...
export const createTargetsState=()=>({kind:'All',adType:'All',period:'30D',query:'',targetWidthRatio:null});
...
export function targetPeriods(state){
  const key=TARGET_PERIODS.includes(state.period)?state.period:'30D';
  const start=key==='ALL'?TARGET_DATE_START:daysBefore(TARGET_DATE_END,Number.parseInt(key)-1);
  return [{key,start,end:TARGET_DATE_END,label:key==='ALL'?`ALL HISTORY · ${start} — ${TARGET_DATE_END}`:`${Number.parseInt(key)} DAYS`,
...
export function filteredTargets(state,status='All'){
  ...
  const filter=node=>{
    if(!node.children.length){
      ...
      return (state.kind==='All'||state.kind===kind)&&(status==='All'||node.status===status)&&(!state.adType||state.adType==='All'||node.adType===state.adType)&&(!query||text.includes(query))?node:null;
    }
